...
const q = computed(() => questions.value[index.value - 1])
...
onMounted(async () => {
  ...
  try {
    ...
    if (first > 0 && index.value > first) return navigateTo(`/diagnostic/${type}/question/${first}`)
...
watch(index, () => (selected.value = answers.value[q.value?.code ?? ''] ?? null))
...
async function next() {
  ...
  try {
    ...
    if (index.value < total.value) await navigateTo(`/diagnostic/${type}/question/${index.value + 1}`)
...
<template>
  <section class="mx-auto max-w-[640px] px-4 py-8">
    <RadarProgressBar :current="index" :total="total" />
    ...
    <div class="mt-8 flex items-center justify-between">
      <NuxtLink v-if="index > 1" :to="`/diagnostic/${type}/question/${index - 1}`" class="min-h-12 px-4 py-3 text-sm font-medium text-gray-600">← Retour</NuxtLink>
      <span v-else />
      <button type="button" :disabled="!selected || saving" class="min-h-12 rounded-xl bg-orange-500 px-6 font-semibold text-white disabled:cursor-not-allowed disabled:opacity-40" @click="next">
        {{ index < total ? 'Suivant' : 'Voir mon résultat' }}
      </button>
    </div>
  </section>
</template>
